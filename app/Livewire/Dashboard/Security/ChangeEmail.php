...
class ChangeEmail extends Component
{
    ...
    public function changeEmailAddress()
    {
        ...
        try {
            if ($this->newEmail !== '' && $this->confirmEmail !== '' && $this->confirmEmail !== $this->newEmail) {
                $this->dispatch('error-message', message: 'New and confirm email fields do not match')->self();
                return;
            }

            if ($this->password !== '' && !Hash::check($this->password, Auth::user()->password)) {
                $this->dispatch('error-message', message: 'Incorrect password')->self();
                return;
            }
