...
class Kyc extends Component
{
  ...
  public function submitKYCApplication()
  {
    ...
    try {
      $pendingKyc = ModelsKyc::where(['user_id' => auth()->user()->id, 'status' => 'pending'])->first();

      if ($pendingKyc) {
        $this->dispatch('error-message', message: 'You have an ongoing KYC request verification and cannot submit another.')->self();
        return;
      }
