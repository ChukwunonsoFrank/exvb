...
class AdminKycDetails extends Component
{
    ...
    {
        try {
            $user = User::where('id', $this->kycRequest->user->id)->first();

            DB::transaction(function () use ($user) {
                User::where('id', $user['id'])->update(['is_kyc_verified' => true]);
                Kyc::where('id', $this->id)->update(['status' => 'approved']);
            });
            ...
            session()->flash('success-message', 'Request approved successfully');
        } catch (\Exception $e) {
    ...
    public function declineRequest()
    {
        try {
           ...
           DB::transaction(function () use ($user) {
                User::where('id', $user['id'])->update(['is_kyc_verified' => false]);
                Kyc::where('id', $this->id)->update(['status' => 'declined']);
            });
            ...
            session()->flash('success-message', 'Request declined successfully');
        } catch (\Exception $e) {
            session()->flash('error-message', $e->getMessage());
        }
    }
